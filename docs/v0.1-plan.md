...
> **Historical design brief.** This is the original v0.1 implementation plan,
> kept for context. It is not maintained: shipped behavior, conventions, and
> hard architectural rules live in `AGENTS.md`. Where the two disagree (for
> example the sidebar and the external-player double-click, both since built),
> `AGENTS.md` and the code are authoritative.
